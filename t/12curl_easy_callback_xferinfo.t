...

{
    ...
    $k |= http::curl_easy_setopt($e, http::CURLOPT_XFERINFOFUNCTION(), my $abc = "");
...

{
    ...
    my $e = http::curl_easy_init();
    http::curl_easy_setopt($e, http::CURLOPT_URL(), 'http://www.example.com/');
    $k |= http::curl_easy_setopt($e, http::CURLOPT_XFERINFOFUNCTION(), my $abc = sub{});
...

{
    ...
    my $e = http::curl_easy_init();
    http::curl_easy_setopt($e, http::CURLOPT_URL(), 'http://www.example.com/');
    $k |= http::curl_easy_setopt($e, http::CURLOPT_XFERINFOFUNCTION(), \&code_sub);
...

{
    ...
    my $e = http::curl_easy_init();
    http::curl_easy_setopt($e, http::CURLOPT_URL(), 'http://www.example.com/');
    $k |= http::curl_easy_setopt($e, http::CURLOPT_XFERINFOFUNCTION(), sub {
        my ($c_e, $cmd, $userp) = @_;
        $e_cnt++ unless defined $c_e and ref($c_e) eq 'http::curl::easy' and "$c_e" eq "$e" and !defined $userp;
        $e_url = http::curl_easy_getinfo($c_e, http::CURLINFO_EFFECTIVE_URL());
        return http::CURLIOE_OK();
    ...
    $k |= http::curl_easy_setopt($e, http::CURLOPT_PUT(), 1);
    $k |= http::curl_easy_setopt($e, http::CURLOPT_NOBODY(), 1);
    $k |= http::curl_easy_setopt($e, http::CURLOPT_XFERINFOFUNCTION(), undef);
    $k |= http::curl_easy_setopt($e, http::CURLOPT_XFERINFOFUNCTION(), sub {});
    $k |= http::curl_easy_setopt($e, http::CURLOPT_XFERINFOFUNCTION());
    $k |= http::curl_easy_setopt($e, http::CURLOPT_XFERINFODATA(), my $dt = []);
    $k |= http::curl_easy_setopt($e, http::CURLOPT_XFERINFODATA(), undef);
    $k |= http::curl_easy_setopt($e, http::CURLOPT_XFERINFODATA(), my $dh = []);
    $k |= http::curl_easy_setopt($e, http::CURLOPT_XFERINFODATA());
    $k |= http::curl_easy_perform($e);
    is($e_cnt, 0, 'callback function called: no errors');
