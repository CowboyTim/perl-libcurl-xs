...

{
    ...
    $k |= http::curl_easy_setopt($e, http::CURLOPT_PROGRESSFUNCTION(), my $abc = "");
...

{
    ...
    my $e = http::curl_easy_init();
    http::curl_easy_setopt($e, http::CURLOPT_URL(), 'http://www.example.com/');
    $k |= http::curl_easy_setopt($e, http::CURLOPT_PROGRESSFUNCTION(), my $abc = sub{});
...

{
    ...
    my $e = http::curl_easy_init();
    http::curl_easy_setopt($e, http::CURLOPT_URL(), 'http://www.example.com/');
    $k |= http::curl_easy_setopt($e, http::CURLOPT_PROGRESSFUNCTION(), \&code_sub);
...

{
    ...
    my $e = http::curl_easy_init();
    http::curl_easy_setopt($e, http::CURLOPT_URL(), 'http://www.example.com/');
    $k |= http::curl_easy_setopt($e, http::CURLOPT_PROGRESSFUNCTION(), sub {
        my ($c_e, $cmd, $userp) = @_;
        $e_cnt++ unless defined $c_e and ref($c_e) eq 'http::curl::easy' and "$c_e" eq "$e" and !defined $userp;
        $e_url = http::curl_easy_getinfo($c_e, http::CURLINFO_EFFECTIVE_URL());
        return http::CURLIOE_OK();
    ...
    $k |= http::curl_easy_setopt($e, http::CURLOPT_PUT(), 1);
    $k |= http::curl_easy_setopt($e, http::CURLOPT_NOBODY(), 1);
    $k |= http::curl_easy_setopt($e, http::CURLOPT_PROGRESSFUNCTION(), undef);
    $k |= http::curl_easy_setopt($e, http::CURLOPT_PROGRESSFUNCTION(), sub {});
    $k |= http::curl_easy_setopt($e, http::CURLOPT_PROGRESSFUNCTION());
    $k |= http::curl_easy_setopt($e, http::CURLOPT_PROGRESSDATA(), my $dt = []);
    $k |= http::curl_easy_setopt($e, http::CURLOPT_PROGRESSDATA(), undef);
    $k |= http::curl_easy_setopt($e, http::CURLOPT_PROGRESSDATA(), my $dh = []);
    $k |= http::curl_easy_setopt($e, http::CURLOPT_PROGRESSDATA());
    $k |= http::curl_easy_perform($e);
    is($e_cnt, 0, 'callback function called: no errors');
